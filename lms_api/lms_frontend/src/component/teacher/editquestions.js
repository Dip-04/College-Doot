...
function EditquizQuestions(){
  ...
  const formSubmit =  (event) => {
    ...
    try {
        axios.post(baseUrl + '/questions/' + question_id+'/' , questiondataformData, {
          headers: {
          ...
          Swal.fire({
            title: 'Questions have been Updated.',
            icon: 'success',
            toast: true,
            timer: 5000,
            position: 'top',
  ...
  useEffect(() => {
    axios
      .get(baseUrl + '/questions/' + question_id)
      .then((response) => {
        setquestiondata({
    ...
    <section className="col-md-9">
        <div className="card">
            <h5 className="card-header text-start">Update Quetions           <Link className="btn btn-success btn-sm  ms-3 float-end" to={'/all-questions/'+quiz_id}>All Questions</Link>
</h5>
...
<div className="mb-3">
  ...
  <input type="text"  name="ans1" value={questiondata.ans1}
